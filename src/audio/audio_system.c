	...
	while (1) {
		...
		*/ 
        ...
        if (CONFIG_AUDIO_SOURCE_USB && CONFIG_AUDIO_DEV == GATEWAY) {
			// Assign USB audio to left channel (mono)
			memcpy(usb_pcm_data + (i * BLOCK_SIZE_BYTES), tmp_pcm_raw_data[i], pcm_block_size);
		} 
		else if (CONFIG_AUDIO_SOURCE_I2S && CONFIG_AUDIO_DEV == GATEWAY) {
			// Assign I2S audio to right channel (mono)
			memcpy(i2s_pcm_data + (i * BLOCK_SIZE_BYTES), tmp_pcm_raw_data[i], pcm_block_size);
		}
